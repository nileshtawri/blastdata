...
% Revision: 0.1
% Descriptions:
% 0.1: Design closed loop CC/CV control for forward mode
...
%Uncompensated voltage loop gain forward CV
%with diode rectifiers
%Vin=800, Vo=400, Tc=582(51.5kHz), G_DC_LLCplant=-16.0418, f0_LLCplant = 2000, Q_db_LLCplant = -4

G_DC_LLCplant=-16.0418; %db
DCgain_LLCplant=(10^(G_DC_LLCplant/20));
%fpole1_LLCplant=4200;
%fpole2_LLCplant=4200;
%fzero1_LLCplant=12000;
...
Tuolfv=GVo_LLCplant*ADC_FS/VoMax*1/(1+s*Rfil_Vo1*Cfil_Vo1)*1/(1+s*Rfil_Vo2*Cfil_Vo2); %uncompensated open loop gain
...
%Uncompensated voltage loop gain forward CV
%with diode rectifiers
%Vin=800, Vo=250, Tc=361(83kHz), G_DC_LLCplant=-0.145596, fpole1_LLCplant = 2.2kHz, fpole2_LLCplant=10kHz, fzero1_LLCplant=28kHz

G_DC_LLCplant1=0.145596; %db
DCgain_LLCplant1=(10^(G_DC_LLCplant1/20));
fpole1_LLCplant1=2300;
fpole2_LLCplant1=9000;
fzero1_LLCplant1=28000;
...
Tuolfv1=GVo_LLCplant1*ADC_FS/VoMax*1/(1+s*Rfil_Vo1*Cfil_Vo1)*1/(1+s*Rfil_Vo2*Cfil_Vo2); %uncompensated open loop gain
...
G_fc=20; %db, shift to get desired fc
KL=1/(10^(G_fc/20));
fzL=2000; % if phase is high at fc, fzL can be placed at high freq
...
G_fc3=-55; %db shift curve upwards to get desired fc (if oscillatory response, reduce gain)
KC=1/(10^(G_fc3/20));
fz3=2500;   %place below (greater) or equal to dominant resonant pole
wz0=2*pi*fz3;
