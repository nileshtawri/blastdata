...
%Uncompensated voltage loop gain forward CC
%with diode rectifiers
%Vin=800, Vo=470, Tc=625(48kHz), G_DC_LLCplant=-17.0763, f0_LLCplant = 1800, Q_db_LLCplant = -4

G_DC_LLCplant=-17.0763; %db
DCgain_LLCplant=(10^(G_DC_LLCplant/20));
%fpole1_LLCplant=2000;
%fpole2_LLCplant=2000;
%fzero1_LLCplant=28000;
...
f0_LLCplant = 1800; %found at 90deg
w0_LLCplant = 2*pi*f0_LLCplant;
fzero1_LLCplant=14000;
...
%Uncompensated voltage loop gain forward CC
%with diode rectifiers
%Vin=800, Vo=250, Tc=361(83kHz), G_DC_LLCplant=-8.89277, fpole1_LLCplant = 2.2kHz, fpole2_LLCplant=7kHz, fzero1_LLCplant=20kHz

G_DC_LLCplant1=-8.89277; %db
DCgain_LLCplant1=(10^(G_DC_LLCplant1/20));
fpole1_LLCplant1=2200;
fpole2_LLCplant1=7000;
fzero1_LLCplant1=20000;
...
% PIT1 KL*(1+(2*pi*fzL)/s) 
G_fc=12; %db, shift to get desired fc
KL=1/(10^(G_fc/20));
fzL=820; % if phase is high at fc, fzL can be placed at high freq
...
G_fc3=-60; %db shift curve upwards to get desired fc (if oscillatory response, reduce gain)
KC=1/(10^(G_fc3/20));
fz3=2200;   %place below (greater) or equal to dominant resonant pole
wz0=2*pi*fz3;
