...
HeaderMatrix = ["KL(dB)" "FZL(Hz)" "a0" "a1" "DC Gain(dB)" "GM(dB)" "GM @(Hz)" "PM(dB)" "PM @(Hz)"];
writematrix(HeaderMatrix,'CVForward470VCompDesign.csv','Delimiter',',')
...
for i = 1:GM_Step_Count+1
    ...
    for j = 1:FzL_Step_Count+1
        ...
        fprintf ('%d. Req. GM %fdB, FzL %fHz: a0=%f, a1=%f, DC_Gain=%f, GM=%fdB, GM at=%fHz, PM=%fdB, PM at=%fHz\n', loopCount, GM_Req_1P1Z, FzL, Num{1,1}(1,1),Num{1,1}(1,2),Kdc,Gm,GmFreq,Pm,PmFreq)
        
        DataMatrix  = [GM_Req_1P1Z FzL Num{1,1}(1,1) Num{1,1}(1,2) Kdc Gm GmFreq Pm PmFreq];
        writematrix(DataMatrix,'CVForward470VCompDesign.csv','WriteMode','append');
